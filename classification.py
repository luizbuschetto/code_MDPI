...
if __name__ == '__main__':
    ...
    X = input_data
    X = X.drop(columns=['final_result'])

    column_names = list(X.columns.values)
    y = input_data['final_result']
    ...
    for i in range(0, len(feature_cols) - questions, NUMBER_OF_FEATURES):
        ...
        y_prediction_naive_bayes = []
        y_prediction_decision_tree = []
        y_prediction_ada_boost = []
        y_prediction_MLP = []
        y_prediction_knn = []
        y_prediction_svm = []
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(X), 1):
            ...
            _y_prediction_random_forest = random_forest.predict(X_test)
            y_prediction_random_forest.append(_y_prediction_random_forest[0])

            naive_bayes.fit(X_train_oversampled, y_train_oversampled)
            _y_prediction_naive_bayes = naive_bayes.predict(X_test)
            y_prediction_naive_bayes.append(_y_prediction_naive_bayes[0])

            decision_tree.fit(X_train_oversampled, y_train_oversampled)
            _y_prediction_decision_tree = decision_tree.predict(X_test)
            y_prediction_decision_tree.append(_y_prediction_decision_tree[0])

            ada_boost.fit(X_train_oversampled, y_train_oversampled)
            _y_prediction_ada_boost = ada_boost.predict(X_test)
            y_prediction_ada_boost.append(_y_prediction_ada_boost[0])

            MLP.fit(X_train_oversampled, y_train_oversampled)
            _y_prediction_MLP = MLP.predict(X_test)
            y_prediction_MLP.append(_y_prediction_MLP[0])

            knn.fit(X_train_oversampled, y_train_oversampled)
            _y_prediction_knn = knn.predict(X_test)
            y_prediction_knn.append(_y_prediction_knn[0])

            svm.fit(X_train_oversampled, y_train_oversampled)
            _y_prediction_svm = svm.predict(X_test)
            y_prediction_svm.append(_y_prediction_svm[0])

            value = _y_test[test_index].values
